...
/// access via `Acervo.isOfflineModeActive`) so this suite is `.serialized` to
/// keep it from racing with any other test that fetches manifests or hits
/// `SecureDownloadSession`. We never make a real network call: the assertion
/// is that the gate throws `.offlineModeActive` BEFORE any URLSession is
/// touched.
@Suite("Offline Mode Gate", .serialized)
