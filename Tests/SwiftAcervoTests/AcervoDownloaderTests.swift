...
extension SharedStaticStateSuite {
  ...
  ///
  /// Nested under `SharedStaticStateSuite` because the test that exercises
  /// real network calls (`downloadFile throws networkError for unreachable URL`)
  /// goes through the offline-mode gate and races against
  /// `OfflineModeGateTests` if both run in parallel. Grandparent `.serialized`
  /// trait eliminates that window.
  @Suite("AcervoDownloader Tests")
