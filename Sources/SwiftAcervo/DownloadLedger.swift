...
// no live `async` continuation at that point, so completion state must be
...
// directory on macOS CI. The iOS background transport (a later phase) writes to
// it from its `URLSessionDownloadDelegate`; the app reads from it on relaunch to
...
public actor DownloadLedger {
  ...
  /// `{ "org/repo": { "model.safetensors": {"verified": {}} } }`.
  private struct Persisted: Codable, Sendable {
    var repos: [String: [String: DownloadFileState]]
  ...
  ///   ``sharedModelsDirectory`` via ``shared(inAppGroup:)``.
  ...
  /// ``DownloadFileState/queued``. Files already tracked keep their current
  /// state (idempotent — safe to call again on relaunch before re-enqueue).
  ...
  /// Sets the state of a single `(repoId, file)`. Creates the entry if absent.
  ...
  public func pendingFiles(repoId: String) -> [String] {
    ...
    return repo.compactMap { $0.value == .verified ? nil : $0.key }.sorted()
  }
  ...
  public func isRepoComplete(repoId: String) -> Bool {
    ...
    return repo.values.allSatisfy { $0 == .verified }
  }
  ...
  public func location(forTaskIdentifier taskIdentifier: Int) -> (repoId: String, file: String)? {
    for (repoId, files) in state.repos {
      for (file, fileState) in files where fileState == .inflight(taskIdentifier: taskIdentifier) {
        return (repoId, file)
      }
    }
    return nil
