...
extension BackgroundDownloadSession: URLSessionDownloadDelegate {
  ...
  public func urlSession(
    ...
    do {
      ...
      Task { await coordinator.recordFailure(taskIdentifier: taskId, reason: "stage failed: \(error)") }
      return
    }
    let taskId = downloadTask.taskIdentifier
  ...
  public func urlSession(
    ...
    Task { await coordinator.recordFailure(taskIdentifier: taskId, reason: String(describing: error)) }
  }
