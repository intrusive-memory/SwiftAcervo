...
/// Metadata for a downloaded AI model.
///
/// Represents a model stored in the shared models directory with its
/// model identifier, local filesystem path, size, and download date.
/// Conforms to `Identifiable` (keyed by model ID), `Equatable`,
...
public struct AcervoModel: Identifiable, Equatable, Codable, Sendable {
  ...
  /// The model identifier in "org/repo" format (e.g., "mlx-community/Qwen2.5-7B-Instruct-4bit").
  ...
  ///
  /// - Parameters:
  ///   - id: The model identifier in "org/repo" format.
